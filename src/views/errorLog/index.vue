<template>
	<div class="app-container calendar-list-container" v-loading="listLoading" element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading" element-loading-background="rgba(0, 0, 0, 0.3)">
		...
		<el-table class="table-size" height="100%" :data="logs" @sort-change="sortChange" fit stripe border highlight-current-row>
			...
			<el-table-column align="center" sortable="custom" width="200" label="时间" prop="OperationTime">
				<template slot-scope="scope">
					<span>{{scope.row.OperationTime | formaTime}}</span>
				</template>
			</el-table-column>
			<el-table-column align="center" label="操作" width="150" fixed="right">
				<template slot-scope="scope">
					<el-button size="small" @click="showError(scope.row)">错误详情
					</el-button>
				</template>
			</el-table-column>
		</el-table>
		...
		<el-dialog title="错误详情" :visible.sync="dialogFormVisible">
			<el-form>
				<el-form-item>
					<el-input type="textarea" :rows="20" disabled v-model="errorDetail"></el-input>
				</el-form-item>
			</el-form>
		</el-dialog>
	</div>
...
<script>
	import { QuickQuery } from '@/api/errorlog'
	...
	export default {
		...
		methods: {
			getList() {
				...
				this.logs = [];
				QuickQuery({
					ServiceName: this.listQuery.Value,
